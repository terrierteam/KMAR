...
class ImprovedRecommendationAdjuster:
    ...
    def __init__(self, user_tastes_file: str = "user_tastes.json", 
        ...
        if openai_api_key:
            ...
            if api_key:
                ...
                print("  Warning: No OpenAI API key provided. GPT analysis will be disabled.")
                self.client = None
        ...
        if self.client:
            ...
            print(f"  Optimized delays: base={self.base_delay}s, max={self.max_delay}s")
            ...
            print("  OpenAI client not available - using enhanced fallback adjustment method")
    ...
    def load_item_summaries(self) -> Dict:
        ...
        try:
            ...
            if isinstance(data, list):
                ...
                print(f"  Unexpected data format for item summaries")
                return {}
    ...
    def _handle_api_error(self, error: Exception) -> bool:
        ...
        if any(phrase in error_str for phrase in ['rate limit', 'too many requests', 'quota exceeded']):
            print(f"  Rate limit hit, increasing delay...")
        ...
        # Other errors - retry once
            ...
            print(f"  Unexpected error, will retry once...")
            self.consecutive_failures += 1
            return True
    ...
    def adjust_recommendations(self, user_ids: List[int] = None, max_users: int = None) -> Dict:
        ...
        if max_users and len(user_ids) > max_users:
            print(f" Limiting processing to {max_users} users (from {len(user_ids)} available)")
        ...
        for i, user_id in enumerate(user_ids):
            ...
            if i % 50 == 0 or i == len(user_ids) - 1:
                ...
                if i > 0:
                    ...
                    print(f"     Elapsed: {elapsed_time/60:.1f}m, Remaining: {estimated_remaining:.1f}m")
                else:
                    print(f"Processing user {i+1}/{len(user_ids)}: User ID {user_id}")
            ...
            try:
                ...
                if not user_taste or not original_list:
                    print(f"    Skipping user {user_id}: missing taste or recommendations")
                    continue
...
Reasoning: Briefly explain your ranking decisions based on the scores and user preferences."""
    ...
    def _parse_enhanced_adjustment_response(self, response_text: str, original_list: List[int]) -> List[int]:
        ...
        try:
            ...
            print(f"   Error parsing enhanced adjustment response: {e}")
...
def main():
    ...
    if args.load_memory and args.memory_file:
        ...
        print(" --load-memory specified but no --memory-file provided")
    ...
    if success:
        ...
        print("\n" + "=" * 60)
        print(" Enhanced Recommendation Adjustment Completed with Warnings!")
        print("=" * 60)
